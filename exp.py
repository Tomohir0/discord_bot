...
class Exp():
    ...
    async def exp_note(self, ctx, number:int):
        ...
        ms=[] # messageのlistを用意
        ms[0] = ("基本的には今まで通り「?notes <label名> <内容>」でmemoを保存して「?calls <label名>」でそのmemoを読み出すことができます。"
        ...
        ms[1] = ("「?notes」でmemoを保存する際、すでにそのlabel名が使用されていた場合には「上書き/付け足し/前に付け足し/label変更/キャンセル」を選択できるようになりました。"
        ...
        ms[2] = ("label名を覚えきれていない場合の補助として、label一覧を見てからlabelを選択できるselcet ver.も用意しました。"
